...
export async function hmacSha1(key, message) {
  const blockSize = 64; // SHA1 block size in bytes
  ...
  if (keyBytes.length > blockSize) {
    const hashedKey = await hashSha1(keyBytes);
  ...
  const innerHash = await hashSha1(innerInput);
  ...
  return await hashSha1(outerInput);
}

// Convenience function for string inputs
export async function hmacSha1String(key, message) {
  return hmacSha1(key, message);
}

// Function to return HMAC as bytes
export async function hmacSha1Bytes(key, message) {
  const hex = await hmacSha1(key, message);

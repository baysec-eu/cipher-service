...
import { hashSha512 } from './hashSha512.js';
...
export async function hmacSha512(key, message) {
  ...
  if (keyBytes.length > blockSize) {
    const hashedKey = await hashSha512(keyBytes);
  ...
  const innerHash = await hashSha512(innerInput);
  ...
  return await hashSha512(outerInput);
}

// Convenience function for string inputs
export async function hmacSha512String(key, message) {
  return hmacSha512(key, message);
}

// Function to return HMAC as bytes
export async function hmacSha512Bytes(key, message) {
  const hex = await hmacSha512(key, message);

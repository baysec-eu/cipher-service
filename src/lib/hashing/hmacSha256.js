...
export async function hmacSha256(key, message) {
  ...
  if (keyBytes.length > blockSize) {
    const hashedKey = await hashSha256(keyBytes);
  ...
  const innerHash = await hashSha256(innerInput);
